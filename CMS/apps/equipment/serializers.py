...
class statusSerializers(serializers.Serializer):
    ...
    type = statusTypeSerializers(label='状态类型', many=False, read_only=True)
...
class equipmentSerializers(serializers.Serializer):
    neid = serializers.CharField(label='neid', read_only=True)
    ...
    unittype = serializers.CharField(label='设备型号')
    # 设备类型
